...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (typeof err?.message === 'string' && /Invalid salt version|Illegal arguments/.test(err.message)) {
      throw createError({ statusCode: 500, message: 'Invalid password hash format in database' })
    }
